#include <gtest/gtest.h>

#include "image_processing_utils/image_processing_utils.hpp"
#include "stereo_banet/banet.hpp"
#include "test_utils/stereo_matching_test_utils.hpp"

using namespace easy_deploy;

#define GEN_TEST_CASES(Tag, FixtureClass)                                                         \
  TEST_F(FixtureClass, test_banet_##Tag##_correctness)                                            \
  {                                                                                               \
    test_stereo_matching_algorithm_correctness(banet_model_, left_image_path_, right_image_path_, \
                                               test_banet_result_path_);                          \
  }                                                                                               \
  TEST_F(FixtureClass, test_banet_##Tag##_async_correctness)                                      \
  {                                                                                               \
    test_stereo_matching_algorithm_async_correctness(banet_model_, left_image_path_,              \
                                                     right_image_path_, test_banet_result_path_); \
  }

class BasebanetFixture : public testing::Test {
protected:
  std::shared_ptr<BaseStereoMatchingModel> banet_model_;

  std::string left_image_path_;
  std::string right_image_path_;
  std::string test_banet_result_path_;
  size_t      speed_test_predict_rounds_;
};

#ifdef ENABLE_TENSORRT

#include "trt_core/trt_core.hpp"

class BANet_TensorRT_Fixture : public BasebanetFixture {
public:
  void SetUp() override
  {
    auto engine = CreateTrtInferCore("/workspace/models/banet-2d-sceneflow-544-960.engine");
    auto preprocess_block = CreateCudaImageProcessingResizePad(ImageProcessingPadMode::TOP_RIGHT,
                                                               ImageProcessingPadValue::EDGE, true,
                                                               true, {0, 0, 0}, {1, 1, 1});

    banet_model_ =
        CreateBANetModel(engine, preprocess_block, 544, 960, {"left", "right"}, {"disp_pred"});

    speed_test_predict_rounds_ = 2000;
    left_image_path_           = "/workspace/test_data/left.png";
    right_image_path_          = "/workspace/test_data/right.png";
    test_banet_result_path_    = "/workspace/test_data/banet_trt_test_result.png";
  }
};

GEN_TEST_CASES(tensorrt, BANet_TensorRT_Fixture);

#endif

#ifdef ENABLE_ORT

#include "ort_core/ort_core.hpp"

class BANet_OnnxRuntime_Fixture : public BasebanetFixture {
public:
  void SetUp() override
  {
    auto engine = CreateOrtInferCore("/workspace/models/banet-2d-sceneflow-544-960.onnx",
                                     {{"left", {1, 3, 544, 960}}, {"right", {1, 3, 544, 960}}},
                                     {{"disp_pred", {1, 1, 544, 960}}});

    auto preprocess_block = CreateCpuImageProcessingResizePad(ImageProcessingPadMode::TOP_RIGHT,
                                                              ImageProcessingPadValue::EDGE, true,
                                                              true, {0, 0, 0}, {1, 1, 1});

    banet_model_ =
        CreateBANetModel(engine, preprocess_block, 544, 960, {"left", "right"}, {"disp_pred"});

    speed_test_predict_rounds_ = 100;
    left_image_path_           = "/workspace/test_data/left.png";
    right_image_path_          = "/workspace/test_data/right.png";
    test_banet_result_path_    = "/workspace/test_data/banet_ort_test_result.png";
  }
};

GEN_TEST_CASES(onnxruntime, BANet_OnnxRuntime_Fixture);

#endif

#ifdef ENABLE_RKNN

#include "rknn_core/rknn_core.hpp"

class BANet_Rknn_Fixture : public BasebanetFixture {
public:
  void SetUp() override
  {
    auto engine = CreateRknnInferCore(
        "/workspace/models/banet-2d-sceneflow-544-960.rknn",
        {{"left_img", RknnInputTensorType::RK_UINT8}, {"right_img", RknnInputTensorType::RK_UINT8}},
        5, 3);
    auto preprocess_block = CreateCpuImageProcessingResizePad(
        ImageProcessingPadMode::TOP_RIGHT, ImageProcessingPadValue::EDGE, false, false, {}, {});

    banet_model_ =
        CreateBANetModel(engine, preprocess_block, 544, 960, {"left", "right"}, {"disp_pred"});
    speed_test_predict_rounds_ = 200;
    left_image_path_           = "/workspace/test_data/left.png";
    right_image_path_          = "/workspace/test_data/right.png";
    test_banet_result_path_    = "/workspace/test_data/banet_rknn_test_result.png";
  }
};

GEN_TEST_CASES(rknn, BANet_Rknn_Fixture);

#endif
